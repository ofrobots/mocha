 ...
 *
 * @param {Error} err
 * @param {boolean|undefined} unhandled - true for unhandled rejections.
 * @api private
 */
Runner.prototype.uncaught = function(err, unhandled) {
  var type = unhandled ? 'unhanled' : 'uncaught';
  if (err) {
    debug(
      '%s exception %s',
      type,
  ...
  if (!runnable) {
    var capitalized = unhandled ? 'Unhandled' : 'Uncaught';
    runnable = new Runnable(capitalized + ' error outside test suite');
...
Runner.prototype.run = function(fn) {
  ...
  function unhandled(reason, promise) {
    self.uncaught(reason, true);
  }
